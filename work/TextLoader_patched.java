...
public class TextLoader {
    ...
    public TextLoader(MyFile fontSheet, MyFile metaFile) {
        // noMipMap: the atlas is large and densely packed; mipmap blending
        // bleeds neighbouring glyphs into each other at smaller text sizes.
        // clampEdges: prevents GL_REPEAT wrap-around sampling at atlas borders.
        this.fontTexture = Texture.newTexture(fontSheet).noFiltering().noMipMap().clampEdges().create();
    ...
    private boolean wordFits(Line line, Word word, Character character, double fontSize) {
        ...
        double space = line.getWords().isEmpty() ? 0.0 : this.metaData.getSpaceWidth() * fontSize;
